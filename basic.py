    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in ' \t':
                ...
                tokens.append(Token(TT_LPARAN, pos_start=self.pos))
                self.advance()
            elif self.current_char == ')':
                tokens.append(Token(TT_RPARAN, pos_start=self.pos))
                self.advance()
            elif self.current_char == '[':
...
class Parser:
    ...
    def comp_expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expectet int, float, identifier, '+', '-', '(', '[' or 'NOT'"
            ))
    ...
    def call(self):
        ...
        if self.current_tok.type == TT_LPARAN:
            res.register_advancement()
            self.advance()
            arg_nodes = []

            if self.current_tok.type == TT_RPARAN:
                res.register_advancement()
                self.advance()
            else:
    ...
    def atom(self):
        ...
        return res.failure(InvalidSyntaxError(
            ...
            "Expectet int, float, identifier, '+', '-', '(', '[', 'IF', 'FOR', 'WHILE', 'FUN'"
        ))
...
global_symbol_table.set('MATH_PI', Number(Number.math_pi))
